...
##   5. Extract motor vehicle-related PM2.5 emission data 
##      in Baltimore City, Maryland and Los Angeles County, California
##   6. Summarize motor vehicle-related PM2.5 emission data in Baltimore City
##      and Los Angeles County by year
...
##   3. subset motor vehicle-related PM2.5 emission data in Baltimore City, 
##      Maryland and Los Angeles County, California
##   3. summarize motor vehicle-related PM2.5 emission data in Baltimore City, 
##      Maryland and Los Angeles County, California by year
##   4. plot 
...
# (fips == "24510") and Los Angeles County, California (fips == "06037")
subData1 <- subset(allData, fips %in% c("24510", "06037"))
# replace fips code with name
# Ref: http://stackoverflow.com/questions/5824173/replace-a-value-in-a-data-frame-based-on-a-conditional-if-statement-in-r
subData1$fips<- as.character(subData1$fips)
subData1$fips[subData1$fips == "06037"] <- "Los Angeles County"
subData1$fips[subData1$fips == "24510"] <- "Baltimore City"
subData1$fips<- as.factor(subData1$fips)

subData2 <- subData1[grep("Mobile .+ Vehicles", subData1$EI.Sector, perl=TRUE),]

# summarize motor vehicle-related PM2.5 emission data in Baltimore City, 
# Maryland and Los Angeles County, California
aggrData <- aggregate(Emissions ~ year + fips, FUN=sum, data=subData2)

## Plot PM2.5 emission trend
library(ggplot2)
# plot coal combustion-related PM2.5 emission data of US
plotObj <- ggplot(aggrData, aes(year, Emissions, color = fips)) + 
...
ggsave(filename="plot6.png", plot=plotObj, dpi=100)
